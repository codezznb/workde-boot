var Wb = {
		...
		return new Ext.ux.window.Notification(Wb.apply({
			...
			listeners: {
				afterrender: {
					...
					fn: function(e) {
						e.header.hide();
						e.mon(e.el, "mousedown", function(f) {
							if (!f.ctrlKey) {
								e.close()
							}
						})
	...
	init: function(a) {
		...
		window.onbeforeunload = Wb.onBeforeUnload = function() {
			if (Wb.unloadEvents) {
				var b, e, c, d = 0;
				Wb.each(Wb.unloadEvents, function(f, g) {
					e = g();
					if (Wb.isValue(e)) {
						if (d === 0) {
							c = Ext.getCmp(f);
		...
		if (window.Ext) {
			if (Ext.isTouch) {
				...
				Ext.getDoc().on("keydown", function(c, b) {
	...
	request: function(b) {
		var f, e, c = Wb.getBool(b.showMask, true),
			d = b.success,
			a = b.failure;
		e = Ext.apply({}, b.params, Wb.getValue(b.out));
			...
			Wb.run({
				...
				success: function(c) {
					c.loginSuccess = function() {
						...
						c.win.destroy();
						Wb.each(c.dockedItems, function(d) {
							d.show()
						});
	...
	prompt: function(g) {
		...
		function h(n, l) {
			...
			if (n.items.getCount() > 0 && l !== null) {
				if (l) {
					...
					Wb.each(n.query("field"), function(o) {
		...
		if (j.windowName) {
			...
			if (j.destroyOn) {
				j.destroyOn.mon(j.destroyOn, "destroy", function() {
					e.destroy();
		...
		function c(f) {
			if (Ext.isTouch) {
				...
				if (f.getRequired && f.getRequired() && f.getValue && g && (!f.isDisabled || !f.isDisabled()) && (!f.getReadOnly || !f.getReadOnly()) && Ext.isEmpty(f.getValue())) {
					a = f;
					Wb.warn(Wb.format(Str.requiredField, d(f)), function() {
						if (f.focus) {
							f.focus(true, true)
						}
					})
					...
					if (g && f instanceof Ext.field.DateTime) {
						...
						if (h && !Ext.isDate(h)) {
							a = f;
							Wb.warn(Wb.format(Str.invalidValue, d(f)), function() {
								f.focus(true, true)
							})
						}
					}
	...
	reload: function(e, d, c) {
		if (e instanceof Ext.grid.Panel || e instanceof Ext.data.Store) {
			var g = e.store || e,
				i = Ext.apply({}, d);
			i.params = Ext.apply({}, i.params, g.lastOptions ? g.lastOptions.params : null);
			g.reload(i)
		} else {
			e = e.bindTable || e;
			if (e.isRefreshing) {
	...
	insertIframe: function(a, b) {
		...
		a.mon(a, "beforedestroy", function(f) {
			var h, i;
			h = f.iframe.getDoc();
			try {
				if (h) {
	...
	edit: function(c, f, d) {
		...
		if (f.win) {
			...
			e.isNew = d;
			e.editData = Wb.apply({}, f.params, d ? null : b.data);
			e.setTitle(f.title);
			...
			if (!d) {
				...
				Wb.setValue(e, Wb.apply({}, f.params, b.data))
			}
		} else {
			h = [];
			if (f.firstItems) {
				Wb.each(f.firstItems, function(j) {
					h.push(j)
				})
			}
			Wb.each(Wb.fetchColumns(a) || Wb.getColumns(a), function(k) {
				var j, l = k.dataIndex;
				if (k.editor) {
					j = Wb.apply({
						itemId: l
			...
			if (f.lastItems) {
				Wb.each(f.lastItems, function(j) {
					h.push(j)
				})
			}
	...
	getData: function(c, d, f) {
		...
		Wb.each(c, function(h) {
			g = Ext.apply({}, h.data);
			if (d) {
				Wb.each(h.data, function(i, j) {
					g["#" + i] = j
				});
				Wb.each(h.modified, function(i, j) {
					g["#" + i] = j
				})
			}
			if (f) {
				e.push(Ext.copyTo({}, h.data, f))
			} else {
				e.push(g)
			}
		});
	...
	clearToolbar: function(c) {
		var b, d, a;
		Ext.suspendLayouts();
		Wb.each(c.items.items, function(e) {
			if (e.hidden) {
				return
			}
	...
	clearFileItem: function(b, a) {
		Wb.each(a.query("filefield"), function(c) {
			if (c.itemId) {
				delete b[c.itemId]
			}
		});
	...
	kvRenderer: window.Ext && Ext.isTouch ? function(c, b) {
	...
	bindChange: function(b, c) {
		...
		if (!b.changeEventBinded) {
			var a = b.store;
			Wb.each(Wb.fetchColumns(b), function(d) {
				if (d.editor) {
	...
	open: function(PortalConfigs) {
		PortalConfigs = Wb.apply({}, PortalConfigs);
	...
	toUrl: function(a) {
		if (Ext.String.endsWith(a, ".xwl")) {
			return "builder?xwl=" + a.slice(0, -4)
		} else {
			return a
		}
	},
	...
	print: function(b, f) {
		...
		e.write('<!DOCTYPE html><html><head><meta http-equiv="content-type" content="text/html;charset=utf-8"/><title>' + (f || "&#160;") + '</title><link type="text/styles" rel="stylesheet" href="wb/styles/style.styles"></head><body onload="this.focus();this.print();" style="font-size:3.174mm;">' + c + "</body></html>");
		e.close()
	},
	printReport: function(a, b, d, c) {
		Wb.report(a, b, function(e) {
			Wb.print(e, d)
		}, c)
	},
	...
	getInputValue: function(a) {
		var c, b = {};
		a = a.el || a.element || a;
		Wb.each(a.query("input"), function(d) {
	...
	setInputValue: function(b, a) {
		var d, c = {};
		b = b.el || b.element || b;
		Wb.each(b.query("input"), function(e) {
	...
	broadcastWithOwner: function(c, f, b) {
		var d, g, e = {},
			a = [];
		(b || Ext.ComponentQuery.query("viewport")[0]).queryBy(function(h) {
			d = h.appScope;
			if (d && !e[d.isXNS]) {
				g = d[c];
				if (Ext.isFunction(g)) {
	...
	highlightCode: function(d, b) {
		var e, a, c = ["java", "js", "css", "html", "sql", "xml", "jsp", "others"],
			g = ["text/x-java", "text/javascript", "text/styles", "text/html", "text/x-plsql", "application/xml", "application/x-jsp", "text/plain"],
			f = function(h) {
				if (!Ext.isIE || !h.up("p")) {
	...
	setNavigate: function(a, b, c) {
		...
		if (Ext.isTouch) {
			Wb.recordActivity(a);
			a.on("activeitemchange", function() {
				var d = this;
				Wb.recordActivity(d)
			})
		} else {
			if (a instanceof Ext.tree.Panel) {
				a.mon(a, "selectionchange", function() {
					var d = this;
					Wb.recordActivity(d)
				})
			} else {
				a.mon(a, "tabchange", function() {
					var d = this;
					Wb.recordActivity(d)
				})
			}
		...
		if (Ext.isTouch) {
			...
			b.mon(b, "click", function(d) {
				Wb.back(d.navComp)
			});
			c.navComp = a;
			c.mon(c, "click", function(d) {
				Wb.forward(d.navComp)
			})
		}
	}
